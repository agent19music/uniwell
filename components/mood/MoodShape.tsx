...
function createMouthSkiaPath(
  ...
  const norm = Math.max(0, Math.min(progress / 5, 1));
...
export function MoodShape({ 
  ...
  // progress: value from 0 to 5, map to shape types
  ...
  useEffect(() => {
    // progress: integer value from 0 to 5
    // 0 -> triangle (stressed/anxious)
    // 1 -> pentagon (tired)
    // 2 -> hexagon (calm)
    // 3 -> Soft-flower blob with 5 smooth bumps (neutral)
    // 4 -> circle (happy)
    // 5 -> Soft-flower blob with 6 smooth bumps (confident)
    const shapeValue = Math.round(progress);
    let target: Float32Array;

    if (shapeValue === 0) {
      // Triangle - stressed/anxious
  ...
  return (
    <Canvas style={{ width: SIZE, height: SIZE }}>
      {/* Main shape */}
      <SkiaPath path={shapePath} color={fillColor} style="fill" />

      {/* left eye */}
      <Circle cx={leftEyeX} cy={eyeY} r={eyeRadius} color={faceColor} />

      {/* right eye */}
      <Circle cx={rightEyeX} cy={eyeY} r={eyeRadius} color={faceColor} />
